...
{if isset($products) && $products}
	...
	<!-- Products list -->
	<div id="load"></div>
	<ul id="{if isset($quick_view) && $quick_view}eb-grid{/if}" class="product_list grid row{if isset($class) && $class} {$class}{/if}{if isset($active) && $active == 1} active{/if} eb-grid" >
			...
			{if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && $product.customizable != 2 && !$PS_CATALOG_MODE}
				{if ($product.allow_oosp || $product.quantity > 0)}
					{if isset($quick_view) && $quick_view}
						<a class="button quick-buy eb-add-cart" href="{$product.link|escape:'html':'UTF-8'}" data-description="{$product.link|escape:'html':'UTF-8'}" title="{l s='Quick view'}">
							<span>{l s='Quick view'}</span>
						</a>
					{else}
						{if isset($static_token)}
							<a class="button ajax_add_to_cart_button btn btn-default eb-cart" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart'}" data-id-product="{$product.id_product|intval}">
...
{/if}
{if $PS_CATALOG_MODE}
	<script src="{$base_uri}js/jquery/plugins/bxslider/jquery.bxslider.js"></script>
{/if}