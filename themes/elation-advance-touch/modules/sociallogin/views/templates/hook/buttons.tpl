...
{foreach from=$social_networks item=item key=k}
	...
	<!--<div class="text-center col-xs-{if !$button}12{elseif $button}4{else}6{/if} col-sm-{if $sign_in && !$button}6{elseif $button}3{else}4{/if} col-md-{if $sign_in && !$button}6{elseif $button}2{else}4{/if}">-->
		<div class="text-center col-xs-3 col-sm-3 col-md-3" style="//padding-left:2px !important;">
			<button style="margin-left:0px !important;" class="btn_social azm-social {$button_class|escape:'html':'UTF-8'} azm-{$item.icon_class|escape:'html':'UTF-8'}" onclick="window.open('{$item.connect|escape:'html':'UTF-8'}', {if $popup}'_blank'{else}'_self'{/if}, 'menubar=no, status=no, copyhistory=no, width=640, height=640, top=220, left=640')" style="color=white!important;">
