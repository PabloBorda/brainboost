...
class totsplashscreen extends Module {
	...
	public function install() {
		...
		if (!Configuration::updateValue('totsplashscreen_delai', '30') OR !Configuration::updateValue('totsplashscreen_version_cookie', '0') OR !Configuration::updateValue('totsplashscreen_fan_page', 'on') OR !Configuration::updateValue('totsplashscreen_fan_page_url', 'https://www.facebook.com/pages/Exclu-Mariagecom/103202581933') OR !Configuration::updateValue('totsplashscreen_newsletter', 'on') OR !Configuration::updateValue('totsplashscreen_text_before', '') OR !Configuration::updateValue('totsplashscreen_count_page', '10')) {
			return false;
		}
	...
	public function uninstall() {
		if (!Configuration::deleteByName('totsplashscreen_delai') OR !Configuration::deleteByName('totsplashscreen_version_cookie') OR !Configuration::deleteByName('totsplashscreen_fan_page') OR !Configuration::deleteByName('totsplashscreen_fan_page_url') OR !Configuration::deleteByName('totsplashscreen_newsletter') OR !Configuration::deleteByName('totsplashscreen_text_before') OR !Configuration::deleteByName('totsplashscreen_count_page')) {
			return false;
		}
	...
	function hookTop($params) {
		...
		if (isset($totsplashscreen['id_totsplashscreen'])) {
			...
			$results = array(
				...
				'image_leave'                         => ($template->image_leave != '' ? $this->getLocalPath() . 'upload/' . $template->image_leave : ''),
				'image_enter'                         => ($template->image_enter != '' ? $this->getLocalPath() . 'upload/' . $template->image_enter : ''),
			);

			$keyCookie = 'totSplashScreen'.$totsplashscreen['id_totsplashscreen'];
			$keyCookieCount = 'totsplashscreen_count_page'.$totsplashscreen['id_totsplashscreen'];

			$newCountValue = isset($totsplashscreenCookie[$keyCookieCount]) ? --$totsplashscreenCookie[$keyCookieCount] : 0;

			if (!isset($totsplashscreenCookie[$keyCookie]) || (isset($totsplashscreenCookie[$keyCookie]) && $totsplashscreenCookie[$keyCookie] < $results['totsplashscreen_version_cookie'])) {

				// Delay
				$delay = (time() + (3600 * 24 * $results['totsplashscreen_delai']));
				// Cookie will be displayed
				if ((isset($totsplashscreenCookie[$keyCookieCount]) && $newCountValue == 0) || $results['totsplashscreen_count_page'] == '0') {
					// Remove count
					if (($results['totsplashscreen_count_page'] == '0' && $totsplashscreen['type'] == 'none') || $newCountValue === 0) {
						setcookie($keyCookieCount, '0', $delay, $url);
					}
					// Cookie version
					setcookie($keyCookie, $results['totsplashscreen_version_cookie'], $delay, $url);
					// If block newsletter
				...
				// Decrements & create
					...
					// For decrements
					if ($results['totsplashscreen_delai'] == 0 && $results['totsplashscreen_count_page'] > 0) {
						$delay += 3600;
					}
					// If cookie doesn't exists
					if (!isset($totsplashscreenCookie[$keyCookieCount]) || (isset($totsplashscreenCookie[$keyCookieCount]) && !is_numeric($totsplashscreenCookie[$keyCookieCount]))) {
						setcookie($keyCookieCount, (int) $results['totsplashscreen_count_page'], $delay, $url);
					}
					else if (isset($totsplashscreenCookie[$keyCookieCount])) {
						setcookie($keyCookieCount, (int) $newCountValue, $delay, $url);
					}
				}
			}
		}
	...
	public function getContent() {
		...
		$this->_postProcess();
		$this->_getProcess();
		if (Tools::getValue('newTotsplashscreenTemplate') || Tools::getValue('editTotsplashscreenTemplate')) {
			//Affichage du formulaire d'ajout de template
			...
			if (version_compare(_PS_VERSION_, '1.5', '<')) {
				$temp .= '<link rel="stylesheet" type="text/css" href="' . $this->getLocalPath() . 'totsplashscreen.css" />';
			}
	...
	public function _getProcess() {
		...
		if (Tools::getValue('viewTotsplashscreenTemplate')) {
			...
			if(version_compare(_PS_VERSION_, '1.5', '<')) {
				global $smarty, $cookie;
				$smarty->assign('fancybox', _PS_JS_DIR_ . 'jquery/jquery.fancybox-1.3.4.js');
				$smarty->assign('fancybox_style', _PS_CSS_DIR_.'jquery.fancybox-1.3.4.css');
				$id_lang = (int)$cookie->id_lang;
			}
			else {
				$smarty = $this->context->smarty;
			...
			$template = new TotSplashScreenTemplateModel(Tools::getValue('viewTotsplashscreenTemplate'), (int)$id_lang);
	...
	public function getLocalPath() {
		if(version_compare(_PS_VERSION_, '1.5', '<')) {
			return _PS_MODULE_DIR_ . $this->name . DIRECTORY_SEPARATOR;
		}
		else {
