...
include_once('ebhomepromos.php');

$context = Context::getContext();
$home_slider = new EBPromoSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
	{
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'ebhomepromos_slides` SET `position` = '.(int)$position.'
			WHERE `id_ebhomepromos_slides` = '.(int)$id_slide
		);
