    ...
    */
    public static function resize(
        ...
        if (!isset($file_type[$src_file])) {
            foreach($list_frames as $src_file => &$frames) {
                foreach ($frames as &$frame) {
                    $frame->delete();
                }
                unset($list_frames[$src_file]);
            }
            unset($list_frames);
            $list_frames = array();
        ...
        /* if (class_exists('Imagick') && method_exists('Imagick', 'getVersion')) { 
            //Se determino que este metodo tarda mucho tiempo aun cuando gasta menos memoria RAM
            ...
            try {
                if (!isset($list_frames[$src_file])) {
                    ...
                    $layers = $gfe->extract($src_file);
                    $list_frames[$src_file] = array();
                    foreach ($layers as &$layer) {
                        $list_frames[$src_file][] = \PHPImageWorkshop\ImageWorkshop::initFromResourceVar($layer['image']);
                    }
                ...
                unset($gc);
                foreach($retouchedFrames as &$img) {
                    imagedestroy($img);
                }
                unset($retouchedFrames);
                return true;
            } catch (Exception $e) {
